...
class DEVO:
    ...
    def load_weights(self, network):
        ...
        self.network.patchify = torch.compile(
            self.network.patchify,
            fullgraph=False,
            backend="inductor",
            options={
        ...
        self.network.update = torch.compile(
            self.network.update,
            fullgraph=False,
            backend="inductor",
            options={
    ...
    def __call__(self, tstamp, image, intrinsics, scale=1.0):
        ...
        if not self.evs:
            ...
            if self.cfg.NORM.lower() == 'none':
                ...
                flatten_image = standardize_events(flatten_image)
            else:
        ...
        if self.n == 8 and not self.is_initialized:
            ...
            return self.poses_[self.n - 1].cpu().clone()

        elif self.is_initialized:
            self.update()
            copy_current_pose = self.poses_[self.n - 1].cpu().clone()
            self.keyframe()

            return copy_current_pose
...
@torch.compile
def standardize_events(flatten_image):
